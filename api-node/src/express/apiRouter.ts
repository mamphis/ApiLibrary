...
export class ApiRouter<
    ...
    DB extends Client,
    C extends Client[K],
    WhereFilter extends Required<
        NonNullable<Parameters<DB[K]["findMany"]>[0]>
    > extends { where?: infer U }
        ? U
        : never,
    UpdateFilter extends Required<
        NonNullable<Parameters<DB[K]["update"]>[0]>
    > extends { data?: infer U }
        ? U
        : never,
    SecurityFilter extends {
        [key in DB[K]["fields"]]: string | number | boolean;
    }
> {
    ...
    private getWhereFilter(req: Request, res: Response): WhereFilter {
        let whereParams: WhereFilter = {} as WhereFilter;
        ...
        if (this.globalFilter) {
            ...
            for (const key in filter) {
                whereFilterSpan.verbose(
                    "{scope} Found field {field} in global filter",
                    { field: key }
                );
        ...
        for (const key in params) {
            if (Object.keys(this.client.fields).includes(key)) {
                whereFilterSpan.verbose(
                    "{scope} Found field {field} directly in params",
                    { field: key }
                );
                ...
                if (Object.keys(this.client.fields).includes(fieldName)) {
                    whereFilterSpan.verbose(
                        "{scope} Found field {field} as object in params",
                        { field: fieldName }
                    );
        ...
        if (search) {
            const searchFilterSpan = whereFilterSpan.startSpan();
            const [searchFilter, searchFilterError] = tryCatch(
                parseSearchParams
            )(search as string);

            if (searchFilterError) {
                searchFilterSpan.error(
                    "{scope} Error parsing search filter: {error}",
                    { error: searchFilterError.message }
                );
            } else {
                searchFilterSpan.verbose(
                    "{scope} Found search filter",
                    searchFilter
                );

                searchFilter.forEach((filter) => {
                    const addFilter = (searchExpression: SearchExpression) => {
                        if (isFieldExpression(searchExpression)) {
                            return addFieldFilter(searchExpression);
                        }

                        if (isObjectExpression(searchExpression)) {
                            return addObjectFilter(searchExpression);
                        }
                    };

                    const addFieldFilter = (fieldFilter: FieldExpression) => {
                        const { field, operator, value } = fieldFilter;

                        if (field in (whereParams as any)) {
                            searchFilterSpan.verbose(
                                "{scope} Found field {field} in where params. Not searching",
                                { field }
                            );
                        } else {
                            searchFilterSpan.verbose(
                                "{scope} Found field {field} in search filter",
                                { field }
                            );
                            const operatorMap = {
                                _eq: "equals",
                    ...
                    const addObjectFilter = (
                        objectFilter: ObjectExpression
                    ): object | undefined => {
                        const { object, value } = objectFilter;

                        if (object in (whereParams as any)) {
                            searchFilterSpan.verbose(
                                "{scope} Found field {field} in where params. Not searching",
                                { field: object }
                            );
        ...
        whereFilterSpan.verbose(
            "{scope} Where params",
            whereParams as Record<string, any>
        );
        return whereParams;
    }
    ...
    getOne(...middlewares: RequestHandler[]): this {
        this.router.get("/:id", ...middlewares, async (req, res, next) => {
            const prepareSpan = req.logger.startSpan(
                "api-" + this.entity.toString()
            );
            prepareSpan.verbose("{scope} Trying to find record with id {id}", {
                id: req.params.id,
            });
            ...
            prepareSpan.verbose("{scope} Found record with id {id}", {
                id: req.params.id,
            });
    ...
    getMany(
        ...
        this.router.get("/", ...middlewares, async (req, res, next) => {
            ...
            databaseSpan.verbose(
                "{scope} loaded {count} of {total} records from database",
                { total, count: recs.length }
            );

            res.json({
                data: await Promise.all(
                    recs.map((rec) =>
                        new this.ctor(rec, this.client).toJsonObject()
                    )
                ),
                total: total,
            });
    ...
    update(
        ...
        this.router.post("/", ...middlewares, async (req, res, next) => {
            ...
            Object.keys(req.body).forEach((key) => {
                ...
                if (value === undefined) {
                    delete req.body[key];
                    setupSpan.verbose("{scope} Removed field {field}", {
                        field: key,
                    });
                }
            });
            ...
            if ("id" in req.body) {
                setupSpan.verbose(
                    "{scope} Trying to find record with id {id}",
                    { id: req.body.id }
                );
                ...
                if (dbRec) {
                    setupSpan.verbose("{scope} Found record with id {id}", {
                        id: req.body.id,
                    });
                    ...
                    await this.db.$transaction(
                        async (client: TransactionClient) => {
                            const init = initializer
                                ? await initializer(req, res, client, req.body)
                                : {};
    ...
    delete(...middlewares: RequestHandler[]): this {
        this.router.delete("/:id", ...middlewares, async (req, res, next) => {
            ...
            try {
                prepareSpan.verbose("{scope} Deleting record with id {id}", {
                    id: req.params.id,
                });
